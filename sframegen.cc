...
const std::complex<float> * sframegen::generate(unsigned char * _payload)
{
    ...
    qpacketmodem_encode(mod, _payload, syms_frame);
    ...
    for (i=0; i<num_symbols_ref; i++)
        firinterp_crcf_execute(interp, syms_ref_0[i], &buf_slot[2*n++]);
    ...
    for (i=0; i<num_symbols_ref; i++)
        firinterp_crcf_execute(interp, syms_ref_1[i], &buf_slot[2*n++]);
